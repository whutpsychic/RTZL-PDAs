...
fc.call = (str) => {
	if (typeof str != "string")
		throw new Error("fc.call函数传入的参数不是一个字符串!");

	if (window.traceback && typeof window.traceback.postMessage == "function")
		window.traceback.postMessage(str);
};
...
fc.traceback = (info) => {
	...
	if (!window.traceback) {
		...
		console.warn(
			"如果你在非flutter端调试，那么traceback方法将不会产生任何作用，且不会影响你继续调试其他功能。"
		);
		return;
	}
